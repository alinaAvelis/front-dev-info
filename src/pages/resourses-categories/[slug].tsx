...
export async function getStaticProps({ params }) {
    ...
    ;
    const categoryQuery = groq`*[_type == "categories_resourses" && activeCategory == true && slug.current == $slug][0]`;
    ...
    return (
        <>
            ...
            <div className="container  container--center">
                <Breadcrumbs pathArr={[{ name: "Категории ресурсов", url: "/resourses-categories" }, {name: category?.title}]} />
				<section className="section tabs mt-50">
					<h1 className="title">{category?.title}</h1>
                    
					<div className="tabs_btns flex ">
                        
						<Cards data={pageData?.filter((point: any) => point?.category?._ref === category?._id)} to="resourses-posts" />
					</div>
				</section>
			</div>
