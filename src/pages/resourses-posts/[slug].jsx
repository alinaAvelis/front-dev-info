...
export async function getStaticProps({ params }) {
	const postQuery = groq`*[_type == "posts_resourses" && active == true && slug.current == $slug][0]`;
...
export const getStaticPaths = async () => {
	const posts = await sanityClient.fetch(
		`*[_type == "posts_resourses" && active == true] { slug }`
	);
	...
	return (
		<>
			<div className="container container--center">
				...
				<Breadcrumbs
					pathArr={[
						{ name: "Посты ресурсов", url: "/resourses-posts" },
						{ name: pageData?.title },
					]}
				/>
