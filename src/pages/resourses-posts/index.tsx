...
export async function getStaticProps() {
	...
	const pageData = await sanityClient.fetch(`*[_type == "posts_resourses"]`);
	const categories = await sanityClient.fetch(`*[_type == "categories_resourses"]`);
...
const AllStories = ({ pageData, categories }) => {
	...
	return (
		<>
			<Head>
				<title>FrontDevInfo - все посты c ресурсами</title>
				<meta
					name="keywords"
					content="программирование, посты, JavaScrip, frontend, ресурсы для программирования"
				/>
				<meta
					name="description"
			...
			<div className="container container--center">
				<Breadcrumbs pathArr={[{ name: "Посты ресурсов", url: "/resourses-posts" }]} />

				<section className="section tabs mt-50">
					<h1 className="title">Все посты с ресурсами</h1>

					<div className="tabs_btns flex ">
						<Cards data={filtredPosts} to="resourses-posts"  />
					</div>
				</section>
			</div>
		</>
